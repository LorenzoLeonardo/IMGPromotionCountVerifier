...
void CCustomClock::CreateClock()
{
	...
	VERIFY(m_cFontAMPM.CreateFont(
		m_nFontSize / 2,                       // nHeight
		0,                        // nWidth
		0,                        // nEscapement
		0,                        // nOrientation
		m_nFontWeight,                // nWeight
	...
	VERIFY(m_cFontDate.CreateFont(
		m_nFontSize / 3,                       // nHeight
		0,                        // nWidth
		0,                        // nEscapement
		0,                        // nOrientation
		m_nFontWeight,                // nWeight
...
void CCustomClock::DrawClock(CClientDC* dcSrc, int x, int y)
{
	...
	dcSrc->SetTextColor(m_textColor);
	dcSrc->SetBkColor(RGB(26, 35, 126));
	dcSrc->TextOut(x, y, csAMPM, csAMPM.GetLength());
	if (sysTime.wHour >= 12)
		csAMPM = _T(" PM");
	else
		csAMPM = _T(" AM");
	dcSrc->TextOut(xInterval, ((tm.tmAscent) / 2) + y, csAMPM, csAMPM.GetLength());
